...
import SQLite_CustomFunc as scf

app = FastAPI()
path = './'

scf.Set_DBName('em3') # DB파일 이름만 넣어주시면 됩니다
...
def recommend_hospital(start_lat, start_lng, emergency, c_id, c_key,hospital_num):
    """
...
def get_hospitals(request:str,latitude:float,longitude:float,hospital_num:int):
    ...
    # 병원 추천
    result = recommend_hospital(latitude, longitude, emergency, c_id, c_key,hospital_num)
    ...
    scf.Log_Insert_2(request, latitude, longitude, predicted_class, hospitalList)
